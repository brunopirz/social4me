      ...
      const res = await fetch("/api/posts/schedule", {
        ...
        body: JSON.stringify({
          ...
          captionOverrides: Object.fromEntries(
            Object.entries(captionOverrides).filter(([, value]) => value.trim())
          ),
        }),
      });
      const json = await res.json();
    ...
    <StudioShell>
      <div className="p-6 md:p-10 max-w-3xl">
        ...
        {selectedAccounts.length > 0 && (
          <div className="mb-6 rounded-2xl border border-white/10 bg-zinc-900/60 p-4">
            <p className="text-sm font-semibold mb-3">Override de legenda por conta</p>
            <div className="space-y-2">
              {accounts
                .filter((account) => selectedAccounts.includes(account.id))
                .map((account) => (
                  <label key={account.id} className="block text-xs text-zinc-400">
                    <span className="mb-1 block">{account.platform} · {account.display_name ?? account.id.slice(0, 8)}</span>
                    <input
                      value={captionOverrides[account.id] ?? ""}
                      onChange={(e) =>
                        setCaptionOverrides((prev) => ({ ...prev, [account.id]: e.target.value }))
                      }
                      placeholder="Opcional: legenda específica para esta conta"
                      className="w-full rounded-xl border border-white/10 bg-zinc-950 px-3 py-2 text-sm text-zinc-300"
                    />
                  </label>
                ))}
            </div>
